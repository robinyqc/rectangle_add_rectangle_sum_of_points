...
def main():
    ...
    parser.add_argument('-N', type=int, default=100000, help='Maximum N.')
    parser.add_argument('-Q', type=int, default=100000, help='Maximum Q.')
    parser.add_argument("-T", type=int, default=20, help="Number of iterations (default: 20)")
    parser.add_argument("-OP", nargs='*', default=["0", "1", "2", "3"], help='Set of operations.')

    
    args = parser.parse_args()

    op = " ".join(map(str, args.OP))
    ...
    for _ in range(args.T):
        # Generate input file
        subprocess.run(f"./{args.C} {args.N} {args.Q} -OP {op} > tc.in", shell=True)
        
        # Measure time and capture output for program A
        time_a, output_a = measure_execution_time(f"./{args.A} < tc.in")
        times_a.append(time_a)
        
        # Measure time and capture output for program B
        time_b, output_b = measure_execution_time(f"./{args.B} < tc.in")
